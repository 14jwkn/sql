...
SELECT * -- I know in real uses * isn't the best - only doing it for efficiency in this assignment.
FROM customer;
...
SELECT vendor_ID, COUNT(*) AS rent_times
...
Thomas Superfood Store, a Fresh Focused store, owned by Thomas Rosenthal
...
and year are! */
SELECT customer_id, 
	STRFTIME('%m',market_date) AS month_purchased, 
	STRFTIME('%Y',market_date) AS year_purchased
FROM customer_purchases; 
...
FROM customer_purchases
WHERE STRFTIME('%m', market_date) = '04' AND STRFTIME('%Y', market_date) = '2022' -- Respecting evaluation order in case it matters.
